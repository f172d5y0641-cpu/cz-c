...
@section('title', 'Buat BAPB Baru')

@section('content')
<h1 class="text-center text-xl font-bold mb-6 text-gray-800">Buat BAPB Baru</h1>

<div class="bg-white p-8 rounded-2xl shadow-md border border-gray-200 max-w-5xl mx-auto">

    <form action="{{ route('vendor.dokumenba.store') }}" method="POST" id="ba-form">
        @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="flex flex-col">
                ...
                <input type="text" name="nama_vendor" id="nama_vendor"
                    class="p-3 rounded-lg border border-gray-300 bg-white focus:ring-2 focus:ring-sky-300"
                    required>
            </div>
            ...
            <div class="flex flex-col md:col-span-2">
                <label for="deskripsi_pekerjaan" class="font-semibold text-gray-700 mb-1">Deskripsi</label>
            ...
            <div class="flex flex-col">
                <label for="nilai_pekerjaan" class="font-semibold text-gray-700 mb-1">Nilai Barang</label>
        ...
        <h3 class="text-lg font-semibold text-gray-800 mb-3">Daftar Barang</h3>

        <table class="w-full border border-gray-300 rounded-xl overflow-hidden">
            <thead>
                <tr class="bg-sky-200 text-gray-800">
                    <th class="p-3 border font-semibold">Nama Barang</th>
                    <th class="p-3 border font-semibold">Merk/Type</th>
                    <th class="p-3 border font-semibold">Jumlah</th>
                    <th class="p-3 border font-semibold">Kondisi</th>
            ...
            <tbody id="barang-table">
                <tr class="bg-white hover:bg-gray-50 transition">
                    ...
                    <td class="p-2 border">
                        <input type="text" name="barang[0][merk]"
                            class="w-full p-2 rounded-md border border-gray-300 bg-white"
                            required>
                    </td>

                    <td class="p-2 border">
                        <input type="number" name="barang[0][jumlah]"
                            class="w-full p-2 rounded-md border border-gray-300 bg-white"
                            required>
                    </td>

                    <td class="p-2 border">
                        <select name="barang[0][kondisi]"
                            class="w-full p-2 rounded-md border border-gray-300 bg-white"
                            required>
        ...
        <div class="flex gap-4 mt-8">
            ...
            <button type="button"
                class="bg-rose-400 hover:bg-rose-500 text-white px-6 py-3 rounded-xl font-semibold"
                onclick="resetForm()">
                Hapus
            </button>
        </div>
...
function tambahRow() {
    ...
    const newRow = `
        <tr class="bg-white hover:bg-gray-50">
            <td class="p-2 border"><input type="text" name="barang[${index}][nama]" class="w-full p-2 rounded-md border border-gray-300 bg-gray-50" required></td>
            <td class="p-2 border"><input type="text" name="barang[${index}][merk]" class="w-full p-2 rounded-md border border-gray-300 bg-gray-50" required></td>
            <td class="p-2 border"><input type="number" name="barang[${index}][jumlah]" class="w-full p-2 rounded-md border border-gray-300 bg-gray-50" required></td>
            <td class="p-2 border">
                <select name="barang[${index}][kondisi]" class="w-full p-2 rounded-md border border-gray-300 bg-gray-50" required>
                    <option value="Baik">Baik</option>
                    <option value="Rusak">Rusak</option>
                </select>
            </td>
            <td class="p-2 border"><input type="text" name="barang[${index}][keterangan]" class="w-full p-2 rounded-md border border-gray-300 bg-gray-50"></td>
            <td class="p-2 border text-center">
                <button type="button" class="bg-red-500 hover:bg-red-600 text-white px-3 py-1 rounded-md" onclick="hapusRow(this)">Hapus</button>
            </td>
        </tr>
    `;
...
function resetForm() {
    const yakin = confirm("Apakah Anda yakin ingin menghapus seluruh data dan mengosongkan form?");
    if (!yakin) return;
