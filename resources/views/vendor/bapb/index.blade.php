...
@section('title', 'Daftar BAPB')

@section('content')
    <h1 class="text-2xl font-bold mb-6 text-gray-800">Daftar BAPB</h1>

    <a href="{{ route('vendor.dokumenba.create', ['jenis' => 'bapb']) }}"
        ...
        <table class="w-full text-sm">
            <thead class="bg-sky-200 text-gray-800">
                <tr>
                    ...
                    <th class="px-4 py-3 text-left">Nilai Barang</th>
            ...
            <tbody>
                @forelse($dokumen as $item)
                    <tr class="border-b hover:bg-gray-50">
                        ...
                        <td class="px-4 py-3">
                            {{ number_format($item->nilai_pekerjaan, 0, ',', '.') }}
                        </td>
                        ...
                        <td class="px-4 py-3 flex gap-2">
                            <a href="{{ isset($item->id_ba) ? route('vendor.dokumen.pdf.preview', $item->id_ba) : '#' }}"
                                class="px-3 py-1 rounded-md bg-sky-600 hover:bg-sky-700 text-white text-xs font-semibold shadow" target="_blank">
                                Preview PDF
                            </a>
                            <a href="{{ route('vendor.dokumenba.show', $item->id_ba) }}"
